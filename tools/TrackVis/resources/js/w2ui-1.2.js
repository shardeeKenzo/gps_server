...
(function () {
	...
	w2grid.prototype = {
		...
		add: function (record) {
			...
			if (this.url == '') {
				this.localSearch();
				if (this.sortOnAdd) this.localSort();
			}
		...
		select: function (recid) {
			...
			for (var a in arguments) {
				var   record  = this.get(arguments[a])
                    , rowName = '#grid_'+this.name +'_rec_'+ w2utils.escapeId(record.recid);
                     
				if (record == null || record.selected === true) continue;
				// event before
                if (!this._selectEventSuppressed) {
                ...
                if (record.confirmed) {
                    if ($(rowName).hasClass('w2ui-odd')) {
                        $(rowName).removeClass('w2ui-odd')
                        $(rowName).addClass('w2ui-odd-confirmed');
                    } else {
                        $(rowName).removeClass('w2ui-even');
                        $(rowName).addClass('w2ui-even-confirmed');
                    }
                } else if (record.cancelled) {
                    if ($(rowName).hasClass('w2ui-odd')) {
                        $(rowName).removeClass('w2ui-odd')
                        $(rowName).addClass('w2ui-odd-cancelled');
                    } else {
                        $(rowName).removeClass('w2ui-even');
                        $(rowName).addClass('w2ui-even-cancelled');
                    }
                } 
                
                if (record.confirmed) {
                    $(rowName).addClass('w2ui-selected-confirmed').data('selected', 'yes');
                } else if (record.cancelled) {
                    $(rowName).addClass('w2ui-selected-cancelled').data('selected', 'yes');
                } else {
				    $(rowName).addClass('w2ui-selected').data('selected', 'yes');
                }
		...
		unselect: function (recid) {
			...
			for (var a in arguments) {
				...
				if (record == null || record.selected !== true) continue;
				// event before
                if (!this._unselectEventSuppressed) {
                ...
                if (record.confirmed) {
                    $('#grid_'+this.name +'_rec_'+ w2utils.escapeId(record.recid)).removeClass('w2ui-selected-confirmed').data('selected', '');
                } else if (record.cancelled) {
                    $('#grid_'+this.name +'_rec_'+ w2utils.escapeId(record.recid)).removeClass('w2ui-selected-cancelled').data('selected', '');
                } else {
		...
		doExpand: function (recid) {
			...
			if (expanded != 'yes') {
				...
				var addClass = ($('#grid_'+this.name +'_rec_'+ w2utils.escapeId(recid)).hasClass(oddclass) ? oddclass : evenclass);
		...
		getRecordsHTML: function () {
			...
			for (var i = startWith, di = 0, ri = 0; ri < this.recordsPerPage && i < this.records.length; i++) {
				...
				if (record.selected) {
					...
					rec_html += '<tr id="grid_'+ this.name +'_rec_'+ record.recid +'" recid="'+ record.recid +'" line="'+ i +'" class="'+ (di%2 == 0 ? oddclass : evenclass) + '" ' +
							(this.isIOS ?
								'	onclick = "w2ui[\''+ this.name +'\'].doDblClick(\''+ record.recid +'\', event);" '
								:
								'	onclick ="w2ui[\''+ this.name +'\'].doClick(\''+ record.recid +'\', event);"'+
...
(function () {
	...
	w2form.prototype = {
		...
		refresh: function () {
            ...
            // refresh header
            var tmp = $(this.box).find('.w2ui-form-header');
            tmp.html(this.header);
